...
class SerialComm(Node):
    ...
    #         raise
        ...
        # publishes wheel velocities (rad/s) read from STM32 encoder data
        self.wheel_pub = self.create_publisher(Float64MultiArray, '/wheel_velocities', 10)

        # receives velocity commands from Nav2 or teleop, forwards to STM32
        self.create_subscription(Twist, '/cmd_vel', self.cmd_vel_callback, 10)

        # read serial in a thread — readline() blocks and would stall the executor
        self._running = True
    ...
    #         # expected format from STM32: "E fl fr rl rr"
    #         # where fl/fr/rl/rr are wheel velocities in rad/s
    #         parts = line.split()
